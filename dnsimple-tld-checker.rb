...
def check_domain(domain_base, tld_suffix, dnsimple_client)
  ...
  if domain_available
    price_check_response = dnsimple_client.registrar.get_domain_prices(ACCOUNT_ID, domain_to_check)
...
ARGV.each do |domain_base|
  matching_endings = tld_suffixes.select{|t| domain_base.end_with?(t)}
  matching_endings.each do |matching_ending|
    check_domain(domain_base.split(matching_ending)[0], matching_ending, dnsimple)
  end
  tld_suffixes.each do |tld_suffix|
    check_domain(domain_base, tld_suffix, dnsimple)
  end
end
